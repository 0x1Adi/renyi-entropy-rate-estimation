...
For a stationary ergodic source with transition matrix $P$, define the Hadamard square $M_2$ by $(M_2)_{ij} = P_{ij}^2$. The $k$-gram collision probability is~\cite{skorski2019}:
\begin{equation}
F_2(k) = \mathbf{v}^\top M_2^{k-1}\, \mathbf{1}, \quad v_i = \pi_i^2
\label{eq:spectral}
\end{equation}
where $\mathbf{1}$ is the all-ones vector. By Perron--Frobenius, $\log F_2(k) \approx k \log \rho(M_2) + C$ for large $k$, where $\rho(M_2)$ is the spectral radius. This gives:
\begin{equation}
h_2 = -\log_2 \rho(M_2) = -\frac{\text{slope}}{\ln 2}
\label{eq:hq}
\end{equation}
...
We use the GraphTunnel DNS dataset~\cite{gao2024graphtunnel}: 124 pcap files spanning 10 classes (82 benign across 2 categories, 42 tunnel across 8 tools). DNS query names are extracted via \texttt{dpkt} and concatenated into a single byte stream per pcap.
...
At $\alpha=16$ with the same multi-scale slope method: R\'enyi $h_2 = 0.720$ vs.\ Shannon rate $= 0.204$ (plug-in) and $0.204$ (Miller-Madow debiased). The advantage ($\Delta\mathrm{ARI} = 0.516$) is intrinsic to the collision functional, not an estimation artifact.
